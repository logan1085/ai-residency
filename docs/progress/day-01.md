...
Day 1 should be hands-on. The reference implementation exists so you have a destination and tests, but the main work is to write the model yourself.
...
| --- | --- | --- | --- |
| Orientation | 20 min | Read Module 1 and the architecture diagram. | Notes on the forward-pass path. |
| Tensor Map | 30 min | Write shapes for `idx`, embeddings, Q/K/V, attention scores, block output, and logits. | Shape table in your notes. |
